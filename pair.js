...
case 'weather': {
    try {
        ...
        if (!city) return await socket.sendMessage(sender, { text: '*🏙️ කරුණාකර නගරයක නම ඇතුළත් කරන්න.*' });

        const res = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=061f24e35386c50d45968411dbc317a9`);
        
        const report = `*🌡️ Weather Report: ${res.data.name}*\n\n` +
                       `*Status:* ${res.data.weather[0].main}\n` +
                       `*Temp:* ${res.data.main.temp}°C\n` +
                       `*Humidity:* ${res.data.main.humidity}%\n` +
                       `*Wind:* ${res.data.wind.speed} m/s`;

        await socket.sendMessage(sender, { text: report }, { quoted: msg });
    } catch (e) {
        await socket.sendMessage(sender, { text: '*❌ නගරය සොයාගත නොහැකි විය.*' });
    }
    break;
}
...
case 'reimagine': {
    try {
        ...
        // 1. පින්තූරයක් Reply කර ඇත්දැයි පරීක්ෂා කිරීම
        if (!quotedMsg || !quotedMsg.imageMessage) {
            return await socket.sendMessage(sender, { text: '*🖼️ කරුණාකර පින්තූරයකට Reply කර (Mention කර) ඔබට අවශ්‍ය වෙනස සඳහන් කරන්න.* \n\n*උදා:* .edit මෙතන කාර් එකක් දාන්න' });
        }
        ...
        if (!prompt) {
            return await socket.sendMessage(sender, { text: '*✍️ පින්තූරය වෙනස් විය යුතු ආකාරය ගැන විස්තරයක් ලබා දෙන්න.*' });
        }

        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        await socket.sendMessage(sender, { text: '*⏳ AI ඔබේ පින්තූරය සකසමින් පවතියි...*' }, { quoted: msg });

        // 2. පින්තූරය Download කර ගැනීම
        ...
        // 3. පින්තූරය Upload කර URL එකක් ලබා ගැනීම (ImgBB හෝ වෙනත් සේවාවක් හරහා)
        // මෙහිදී පහසුව සඳහා පින්තූරය සහ Prompt එක එකවර හසුරුවන API එකක් භාවිතා කරයි.
        const base64Image = buffer.toString('base64');
        
        // සටහන: Image editing සඳහා සාමාන්‍යයෙන් Stable Diffusion Inpainting API එකක් අවශ්‍ය වේ.
        // පහත දැක්වෙන්නේ පොදු API එකක් භාවිතා කරන ආකාරයයි.
        const apiUrl = `https://api.giftedtech.my.id/api/ai/reimagine?prompt=${encodeURIComponent(prompt)}&url=${encodeURIComponent('BASE64_OR_URL_HERE')}`;
        
        // විස්තර සඳහා දත්ත සකස් කිරීම
        const timeNow = new Date().toLocaleString('en-US', { timeZone: 'Asia/Colombo' });
        const requester = msg.pushName || 'User';

        const captionText = `*CHALAH MD ❗*\n\n` +
                          `*📝 විස්තරය:* ${prompt}\n` +
                          `*👤 ඉල්ලුම්කරු:* ${requester}\n` +
                          `*⏰ වේලාව:* ${timeNow}\n\n` +
                          `*✨ AI මගින් සාර්ථකව නිම කරන ලදී.*`;

        // 4. ප්‍රතිඵලය යැවීම
        // සැබෑ භාවිතයේදී මෙහිදී API එකෙන් ලැබෙන රූපය (Result Image) ලබාගත යුතුය.
        await socket.sendMessage(sender, { 
            image: { url: `https://api.giftedtech.my.id/api/ai/reimagine?prompt=${encodeURIComponent(prompt)}` }, // මෙහි ඔබේ API එක යොදන්න
            caption: captionText 
        }, { quoted: msg });

    } catch (e) {
        console.log(e);
        await socket.sendMessage(sender, { text: '*❌ පින්තූරය සකස් කිරීමේදී දෝෂයක් සිදු විය. කරුණාකර පසුව උත්සාහ කරන්න.*' });
    }
    break;
}
...
case 'hw': {
    try {
        const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';
        const words = text.split(" ").slice(1).join(" ").trim();
        if (!words) return await socket.sendMessage(sender, { text: '*✍️ ලියන්න ඕනි දේ ඇතුළත් කරන්න.*' });

        await socket.sendMessage(sender, { react: { text: '📝', key: msg.key } });

        const hwUrl = `https://api.giftedtech.my.id/api/tools/handwriting?text=${encodeURIComponent(words)}`;

        await socket.sendMessage(sender, { 
            image: { url: hwUrl }, 
            caption: `*CHALAH MD ❗*\n\n*අතින් ලියන ලද පිටපත සූදානම්.*` 
        }, { quoted: msg });
    } catch (e) {
        await socket.sendMessage(sender, { text: '*❌ ලිවීමට නොහැකි විය.*' });
    }
    break;
}
...
case 'sing': {
    const text = msg.message.conversation || msg.message.extendedTextMessage?.text || '';
    const lyrics = text.split(" ").slice(1).join(" ").trim();
    if (!lyrics) return await socket.sendMessage(sender, { text: '*🎶 ගායනා කළ යුතු පද පේළි ලබා දෙන්න.*' });

    await socket.sendMessage(sender, { react: { text: '🎤', key: msg.key } });
    
    const singUrl = `https://api.giftedtech.my.id/api/ai/lyrics-to-voice?text=${encodeURIComponent(lyrics)}`;
